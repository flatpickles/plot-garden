...
### Summary
Build `/Users/matt/dev/vibe-plotter` as a `pnpm`-managed Next.js App Router project where each sketch is a typed `PlotterSketch` subclass in `/Users/matt/dev/vibe-plotter/src/sketches`, automatically listed in a searchable UI, parameterized via a control panel, rendered as SVG, and optionally sent directly to AxiDraw/EBB hardware via Web Serial on Chromium browsers. Non-Chromium desktop browsers still get full browse/render/export functionality with clear plotting fallback messaging.  
Execution will start by writing this plan to `/Users/matt/dev/vibe-plotter/PLAN.md`.

### Milestone 0 — Plan Capture
Create `/Users/matt/dev/vibe-plotter/PLAN.md` with this full plan verbatim as the first execution step.

### Milestone 1 — Project Bootstrap (`pnpm`, Next.js, baseline tooling)
Initialize Next.js (App Router + TypeScript) in `/Users/matt/dev/vibe-plotter` using `pnpm`, add lint/test scripts, and set strict TS defaults.  
Set up baseline directories: `/Users/matt/dev/vibe-plotter/src/app`, `/Users/matt/dev/vibe-plotter/src/lib`, `/Users/matt/dev/vibe-plotter/src/sketches`, `/Users/matt/dev/vibe-plotter/scripts`, `/Users/matt/dev/vibe-plotter/src/generated`.  
...
Implement abstract base class `PlotterSketch` and shared types in `/Users/matt/dev/vibe-plotter/src/lib/sketch-core`.  
...
- `/Users/matt/dev/vibe-plotter/src/sketches/<slug>/<Slug>.ts` (class extending `PlotterSketch`)
- `/Users/matt/dev/vibe-plotter/src/sketches/<slug>/sketch.json` (manifest)
- `/Users/matt/dev/vibe-plotter/src/sketches/<slug>/thumbnail.png` (script-generated)
Implement `zod` validation for `sketch.json`.  
Implement `pnpm sketch:new <slug>` generator in `/Users/matt/dev/vibe-plotter/scripts/new-sketch.ts` to scaffold class + manifest + starter thumbnail placeholder.  
Implement `pnpm sketches:sync` in `/Users/matt/dev/vibe-plotter/scripts/sync-sketches.ts` to validate manifests and generate `/Users/matt/dev/vibe-plotter/src/generated/sketch-registry.ts`.  
...
Build a two-pane layout in `/Users/matt/dev/vibe-plotter/src/app` with:
...
Implement EBB transport abstraction and Chromium implementation in `/Users/matt/dev/vibe-plotter/src/lib/plotter` using Web Serial:
- Connect/disconnect.
...
Implement `pnpm sketches:thumbnails` in `/Users/matt/dev/vibe-plotter/scripts/generate-thumbnails.ts` to regenerate committed previews for all sketches.  
Add docs:
- `/Users/matt/dev/vibe-plotter/README.md` quickstart and architecture.
- `/Users/matt/dev/vibe-plotter/docs/SKETCH_API.md` sketch contract + layer conventions.
- `/Users/matt/dev/vibe-plotter/docs/PLOTTER_SUPPORT.md` browser support and hardware notes.
Add CI-friendly tests and smoke checks.

## Public APIs, Interfaces, and Types (New/Changed)

### Sketch Authoring API
`PlotterSketch` abstract class in `/Users/matt/dev/vibe-plotter/src/lib/sketch-core/PlotterSketch.ts`.  
...
### Manifest API
`sketch.json` schema in `/Users/matt/dev/vibe-plotter/src/lib/sketch-core/manifestSchema.ts` with slug/title/description/tags/order/thumbnail path metadata.

### Registry API
Generated module `/Users/matt/dev/vibe-plotter/src/generated/sketch-registry.ts` exporting typed sketch descriptors for UI and routing.
...
### Unit Tests
`/Users/matt/dev/vibe-plotter/src/lib/sketch-core/*.test.ts` for schema validation, output normalization, SVG top-level-group layer extraction, param coercion, and seed determinism.  
`/Users/matt/dev/vibe-plotter/src/lib/plotter/*.test.ts` for planner ordering, flatten mode, pause-between-layer markers, repeat expansion, and EBB command encoding rules.  
`/Users/matt/dev/vibe-plotter/scripts/*.test.ts` for scaffold/sync/thumbnail script behavior and validation errors.
